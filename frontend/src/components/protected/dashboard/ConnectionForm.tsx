...
const ConnectionForm = ({
  ...
  const [createDashboardEntry, { isLoading: isCreating }] =
    useCreateDashboardEntryMutation();
    ...
    onSubmit: async (values) => {
      ...
      try {
        if (editForm && formData) {
          ...
          await createDashboardEntry(payload).unwrap();
        }
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="space-y-1">
          ...
          {formik.touched.host && formik.errors.host && (
            <div className="text-sm text-destructive">{formik.errors.host}</div>
          )}
        </div>
        ...
        <div className="space-y-1">
          ...
          {formik.touched.port && formik.errors.port && (
            <div className="text-sm text-destructive">{formik.errors.port}</div>
          )}
        </div>
      </div>
